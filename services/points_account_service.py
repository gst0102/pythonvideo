...
class PointsAccountService:
    ...
    @staticmethod
    async def move_frozen_to_consumable(
        session: AsyncSession,
        user_id: UUID,
        points: int,
        idempotency_key: str,
        ...
        *,
        source: str = "netdisk",
        change_type: str = "frozen_to_consumable",
    ) -> Tuple[PointsLedger, UserAccount, bool]:
        existing = await PointsAccountService.get_ledger_by_idempotency_key(session, idempotency_key)
        account, _ = await PointsAccountService.ensure_user_account(session, user_id)
        if existing:
    ...
    async def deduct_frozen_points(
        ...
        *,
        source: str = "netdisk",
        change_type: str = "frozen_reward_clawback",
    ) -> Tuple[PointsLedger, UserAccount, bool]:
        existing = await PointsAccountService.get_ledger_by_idempotency_key(session, idempotency_key)
        account, _ = await PointsAccountService.ensure_user_account(session, user_id)
        if existing:
    ...
    @staticmethod
    async def move_consumable_to_withdrawable(
        session: AsyncSession,
        user_id: UUID,
        points: int,
        idempotency_key: str,
        ...
        *,
        source: str = "game",
        change_type: str = "game_settlement",
    ) -> Tuple[PointsLedger, UserAccount, bool]:
        existing = await PointsAccountService.get_ledger_by_idempotency_key(session, idempotency_key)
        account, _ = await PointsAccountService.ensure_user_account(session, user_id)
        if existing:
    ...
    @staticmethod
    async def consume_consumable_points(
        session: AsyncSession,
        user_id: UUID,
        points: int,
        idempotency_key: str,
        ...
        *,
        source: str = "netdisk",
        change_type: str = "resource_unlock",
    ) -> Tuple[PointsLedger, UserAccount, bool]:
        existing = await PointsAccountService.get_ledger_by_idempotency_key(session, idempotency_key)
        account, _ = await PointsAccountService.ensure_user_account(session, user_id)
        if existing:
    ...
    @staticmethod
    async def adjust_withdrawable_points(
        session: AsyncSession,
        user_id: UUID,
        points_delta: int,
        idempotency_key: str,
        ...
        *,
        ...
        if delta < 0 and int(account.withdrawable_points) < abs(delta):
            raise ValueError("insufficient withdrawable points")
