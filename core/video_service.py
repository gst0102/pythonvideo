...
from core.reverse_service import ReverseService
...
class VideoService:
    ...
    def get_video_info(cls, video_url: str) -> dict:
        if ReverseService.is_supported(video_url):
            return ReverseService.get_video_info(video_url)
    ...
    def create_stream_generator(cls, video_url: str, format_preset: str = "fast") -> Tuple[str, Iterator[bytes], dict]:
        ...
        if ReverseService.is_supported(video_url):
            return cls._create_reverse_stream_generator(video_url, format_preset)
    ...
    @classmethod
    def _create_reverse_stream_generator(cls, video_url: str, format_preset: str = "fast") -> Tuple[str, Iterator[bytes], dict]:
        stream_info = ReverseService.extract(video_url, format_preset)
        ...
        source_resp, source_headers = cls.fetch_stream(stream_url, request_headers=request_headers)

        def stream_generator() -> Iterator[bytes]:
            try:
                logger.info("开始转发 %s 视频流: %s", platform, video_title)
                for chunk in source_resp.raw.stream(65536, decode_content=True):
                    if chunk:
                        yield chunk.tobytes() if hasattr(chunk, "tobytes") else chunk
                logger.info("%s 视频流传输完成: %s", platform, video_title)
            finally:
                source_resp.close()
                logger.info("%s 源站连接已关闭: %s", platform, video_title)
