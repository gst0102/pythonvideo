...
- 驳回误投诉后，如果资源未驳回投诉数低于阈值，系统可自动恢复资源可见
- 后台支持对被隐藏资源手动恢复上架，用于误投诉、误判或补链后恢复
...
- 上传冻结奖励、补链冻结奖励、投诉隐藏阈值、失效处罚倍数应放入后台配置 `netdisk_audit_config`
- MVP 默认值：上传冻结 +5、补链冻结 +5、3 条投诉隐藏、失效处罚 1 倍
- 后续如要加强处罚，可先通过后台配置调整倍数，不直接改代码
- 配置变更只影响后续新提交或新处罚，不强行改写历史流水

待追缴 / 风控口径：

- 确认失效处罚时，如果用户可用积分不足，先扣除当前可扣的可用积分
- 不足部分写入 `netdisk_risk_records`，状态为 `open`
- 待追缴记录用于后续人工处理、风控观察或自动追扣任务
- 本阶段只记录待追缴，不自动从用户后续收入中追扣
